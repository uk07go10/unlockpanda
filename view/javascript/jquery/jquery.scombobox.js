...
(function($, document) {
    ...
    var methods = {
        ...
        rebuildDict: function () {
            dict = {};
            var $options = this.find("select option");
            $options.each(function (index) {
                var $self = $(this);
                var text = $self.text().trim().toLowerCase();
                var metadata = $self.attr("data-metadata");
                if(typeof(metadata) !== "undefined") {
                    text += (" " + metadata.toLowerCase());
                }

                if (!(text in dict)) {
                    dict[text] = [];
                }
                dict[text].push(index);
            });
        },
        init: function () {
    ...
    function addListeners() {
        ...
        this.on('keyup', cp + cdisplay + ', ' + cp + cdiv, function(e) { // filter
            ...
            var doneTyping = function(e) {
                ...
                for (var index in dict) {
                    if (index.indexOf(search) >= 0) {
                        dict[index].forEach(function (value) {
                            $children.eq(value).show();
                        });
                    }
                }
                ...
                //
                //        var $ps = $div.children().eq($options.index(this)).show();
                //        if (highlight) {
                //            $ps.each(function() {
        ...
        this.on('blur', cp + cdisplay, function(e) {
          ...
          */
          ...
          if (previousV !== $valueInput.val() || true) {
