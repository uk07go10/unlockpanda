...
class ModelCatalogTestimonial extends Model {
	...
	public function getLastTestimonial() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "testimonial t LEFT JOIN " . DB_PREFIX . "testimonial_description td ON (t.testimonial_id = td.testimonial_id) WHERE t.testimonial_id = (select max(testimonial_id) from testimonial where status='1') AND td.language_id = '" . (int)$this->config->get('config_language_id') . "' AND t.status = '1'");
		return $query->row;
	}
